...
import { Cliente, Electrodomestico } from './clientePromises'
...
describe('test del cliente', () => {
  ...
  test('promises - Compra exitosa de un LCD TV barata por debajo del saldo del cliente', () => {
    ...
    return cliente.procesoDeCompra(electrodomestico, 1200)
      .then(() => {
  ...
  test('promises - Compra fallida, tengo plata pero para mi consideración la LCD TV es muy cara', () => {
    const cliente = new Cliente(2000)
    return cliente.procesoDeCompra(electrodomestico, 800)
      .then(() => { throw new Error('No debería haber comprado') })
      .catch((message: unknown) => {
        expect((message as Error).message).toBe('Mmm... no me convence pagar más de $ 800 por un/a LCD TV')
